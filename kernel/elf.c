...
elf_sect_header get_debug_line_section(elf_ctx *ctx)
{
    ...
    for (int i = 0; i < ctx->ehdr.shnum; i++)
    {
        ...
        // sprint("%s\n", sstr + name);
        if (strcmp(sstr + name, ".debug_line") == 0)
        {
            elf_fpread(ctx, (void *)&debug_line, sizeof(elf_sect_header), offset);
        }
    }
    return debug_line;
}
...
elf_status elf_load(elf_ctx *ctx)
{
    ...
    // elf_sect_header debug_line = get_debug_line_section(ctx);
    //  获取Section header string table index的偏移地址
    uint64 offset_shstrndx = ctx->ehdr.shoff + ctx->ehdr.shentsize * ctx->ehdr.shstrndx;
    elf_sect_header shstr;
    elf_fpread(ctx, (void *)&shstr, (uint64)ctx->ehdr.shentsize, offset_shstrndx);
    ...
    elf_fpread(ctx, (void *)content, debug_line.size, debug_line.offset);
    make_addr_line(ctx, content, debug_line.size);
    // sprint("%d\n", debug_line.size);
