...
void load_user_program(process *proc)
{
  ...
  proc->trapframe[hartid] = (trapframe *)alloc_page();
  memset(proc->trapframe[hartid], 0, sizeof(trapframe));

  // allocate a page to store page directory. added @lab2_1
  proc->pagetable[hartid] = (pagetable_t)alloc_page();
  memset((void *)proc->pagetable[hartid], 0, PGSIZE);

  // allocate pages to both user-kernel stack and user app itself. added @lab2_1
  proc->kstack[hartid] = (uint64)alloc_page() + PGSIZE; // user kernel stack top
  uint64 user_stack = (uint64)alloc_page();             // phisical address of user stack bottom

  // USER_STACK_TOP = 0x7ffff000, defined in kernel/memlayout.h
  proc->trapframe[hartid]->regs.sp = USER_STACK_TOP; // virtual address of user stack top

  sprint("hartid = %d: user frame 0x%lx, user stack 0x%lx, user kstack 0x%lx \n", hartid, proc->trapframe[hartid],
         proc->trapframe[hartid]->regs.sp, proc->kstack[hartid]);
  ...
  user_vm_map((pagetable_t)proc->pagetable[hartid], USER_STACK_TOP - PGSIZE, PGSIZE, user_stack,
              prot_to_type(PROT_WRITE | PROT_READ, 1));

  // map trapframe in user space (direct mapping as in kernel space).
  user_vm_map((pagetable_t)proc->pagetable[hartid], (uint64)proc->trapframe[hartid], PGSIZE, (uint64)proc->trapframe[hartid],
              prot_to_type(PROT_WRITE | PROT_READ, 0));

  // map S-mode trap vector section in user space (direct mapping as in kernel space)
  // here, we assume that the size of usertrap.S is smaller than a page.
  user_vm_map((pagetable_t)proc->pagetable[hartid], (uint64)trap_sec_start, PGSIZE, (uint64)trap_sec_start,
...
int s_start(void)
{
  ...
  sync_barrier((int *)&counter_s, NCPU);
